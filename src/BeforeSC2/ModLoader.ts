...
export class ModImgGetterDefault implements IModImgGetter {
  constructor(public modName: string, public zip: ModZipReader, public imgPath: string, public logger: LogWrapper) {}
  ...
  async getBase64Image() {
    ...
    console.error(
      `ModImgGetterDefault getBase64Image() imgFile not found: ${this.imgPath} in ${this.zip.modInfo?.name}`
    );
    this.logger.error(
      `ModImgGetterDefault getBase64Image() imgFile not found: ${this.imgPath} in ${this.zip.modInfo?.name}`
    );
...
export function checkModBootJsonAddonPlugin(v: any): v is ModBootJsonAddonPlugin {
  let c: boolean = isString(get(v, "modName")) && isString(get(v, "addonName")) && isString(get(v, "modVersion"));
  if (c && has(v, "params")) {
...
export function checkDependenceInfo(v: any): v is DependenceInfo {
  return isString(get(v, "modName")) && isString(get(v, "version"));
}
...
export class ModLoader {
  ...
  private async do_initModInjectEarlyLoadInDomScript(modName: string, mod: ModInfo) {
    for (const [name, content] of mod.scriptFileList_inject_early) {
      ...
      if (this.gSC2DataManager) {
        ...
        // or insert to head
        console.warn(
          "ModLoader ====== do_initModInjectEarlyLoadInDomScript() gSC2DataManager is undefined, insert to head"
        );
        this.logger.warn(
          `ModLoader ====== do_initModInjectEarlyLoadInDomScript() gSC2DataManager is undefined, insert to head`
        );
  ...
  private async do_initModEarlyLoadScript(modName: string, mod: ModInfo) {
    for (const [name, content] of mod.scriptFileList_earlyload) {
      ...
      try {
        ...
        const R = await this.gSC2DataManager
          .getJsPreloader()
          .JsRunner(content, name, modName, "EarlyLoadScript", this.gSC2DataManager, this.thisWin, this.logger);
  ...
  private async initModEarlyLoadScript() {
    ...
    if (!this.loadEndModList.checkNameUniq()) {
      // never go there
      console.error(
        "ModLoader ====== initModEarlyLoadScript() loadEndModList.checkNameUniq() failed. Data consistency check failed.",
        [this.loadEndModList]
      );
      this.logger.error(
        `ModLoader ====== initModEarlyLoadScript() loadEndModList.checkNameUniq() failed. Data consistency check failed.`
  ...
  public getModEarlyLoadCache(): ModOrderContainer {
    if (this.modCache.size > 0) {
      ...
      if (
        !isNil(this.loadEndModList) ||
        !isNil(this.nowLoadedMod) ||
        !isNil(this.toLoadModList) ||
        !isNil(this.newNowMod) ||
        !isNil(this.replacedNowMod)
      ) {
        // invalid state, never go there
    ...
    if (this.replacedNowMod) {
      return ModOrderContainer.mergeModOrderContainer([this.loadEndModList!, this.newNowMod!, this.nowLoadedMod!]);
    } else {
      return ModOrderContainer.mergeModOrderContainer([this.loadEndModList!, this.nowLoadedMod!]);
    }
  }
  ...
  private async tryInitWaitingLazyLoadMod(
    ...
    if (this.modLazyCache.size > 0) {
      ...
      const checkCanSafeOverwriteMod = (mod: ModOrderItem) => {
        return (
          nowMod.name === mod.name || this.toLoadModList!.getHasByName(mod.name)
      ...
      while (this.modLazyCache.size > 0) {
        ...
        if (checkCanSafeOverwriteMod(mod)) {
          ...
          this.logger.log(
            `ModLoader ====== tryInitWaitingLazyLoadMod() mod overwrite safe already loaded: [${mod.name}]. when LazyLoad by [${nowMod.name}] . ` +
          ...
          this.logger.warn(
            `ModLoader ====== tryInitWaitingLazyLoadMod() mod overwrite unsafe already loaded: [${mod.name}]. when LazyLoad by [${nowMod.name}] . ` +
        ...
        if (this.loadEndModList!.getHasByName(mod.name)) {
          ...
          this.logger.warn(
            `ModLoader ====== tryInitWaitingLazyLoadMod() overwrite loaded mod: [${mod.name}]. when LazyLoad by [${nowMod.name}] . ` +
        ...
        if (this.toLoadModList!.getHasByName(mod.name)) {
          ...
          this.logger.warn(
            `ModLoader ====== tryInitWaitingLazyLoadMod() overwrite later mod: [${mod.name}]. when LazyLoad by [${nowMod.name}] . ` +
        ...
        if (this.nowLoadedMod.getHasByName(mod.name)) {
          ...
          this.logger.log(
            `ModLoader ====== tryInitWaitingLazyLoadMod() overwrite loaded lazy mod: [${mod.name}]. when LazyLoad by [${nowMod.name}] . ` +
