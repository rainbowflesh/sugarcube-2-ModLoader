...
export class HtmlTagSrcHook {
  ...
  constructor(public gSC2DataManager: SC2DataManager) {
  ...
  private hookReturnModeTable: Map<string, HtmlTagSrcReturnModeHookType> = new Map<
    string,
    HtmlTagSrcReturnModeHookType
  >();
  ...
  public checkImageExist(src: string): boolean | undefined {
    ...
    for (const [hookKey, hook] of this.hookCheckExistTable) {
      try {
        ...
        this.logger.error(
          `[HtmlTagSrcHook] checkImageExist: call hookKey[${hookKey}] error [${e?.message ? e.message : e}]`
        );
      }
    }
  ...
  public async doHook(el: HTMLImageElement | HTMLElement, field: string = "src"): Promise<boolean> {
    ...
    for (const [hookKey, hook] of this.hookReturnModeTable) {
      try {
        ...
        this.logger.error(
          `[HtmlTagSrcHook] doHookCallback: call hookKey[${hookKey}] error [${e?.message ? e.message : e}]`
        );
      }
    }
  ...
  async requestImageBySrc(src: string) {
    ...
    for (const [hookKey, hook] of this.hookReturnModeTable) {
      try {
        ...
        this.logger.error(
          `[HtmlTagSrcHook] requestImageBySrc: call hookKey[${hookKey}] error [${e?.message ? e.message : e}]`
        );
      }
    }
    // if no one can handle the element, do the default action
