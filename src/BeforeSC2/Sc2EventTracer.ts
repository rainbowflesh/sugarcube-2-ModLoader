...
export interface Sc2EventTracerCallback {
  // after SugarCube2 trigger StoryReady event
  whenSC2StoryReady?: () => any;

  // after SugarCube2 trigger PassageInit event
  whenSC2PassageInit?: (passage: Passage) => any;

  // after SugarCube2 trigger PassageStart event
  whenSC2PassageStart?: (passage: Passage, content: HTMLDivElement) => any;

  // after SugarCube2 trigger PassageRender event
  whenSC2PassageRender?: (passage: Passage, content: HTMLDivElement) => any;

  // after SugarCube2 trigger PassageDisplay event
  whenSC2PassageDisplay?: (passage: Passage, content: HTMLDivElement) => any;

  // after SugarCube2 trigger PassageReady event
...
export class Sc2EventTracer {
  constructor(public thisWin: Window, public gSC2DataManager: SC2DataManager) {}

  callback: Sc2EventTracerCallback[] = [];

  callbackLog: [
    string,
    WeakPoolRef<Sc2EventTracerCallback>,
    ...WeakPoolRef<any>[]
  ][] = [];

  init() {
    this.thisWin
      .jQuery(this.thisWin.document)
      .on(":storyready", (event: Event | any) => {
    ...
    this.thisWin
      .jQuery(this.thisWin.document)
      .on(":passageinit", (event: Event | any) => {
    ...
    this.thisWin
      .jQuery(this.thisWin.document)
      .on(":passagestart", (event: Event | any) => {
    ...
    this.thisWin
      .jQuery(this.thisWin.document)
      .on(":passagerender", (event: Event | any) => {
    ...
    this.thisWin
      .jQuery(this.thisWin.document)
      .on(":passagedisplay", (event: Event | any) => {
