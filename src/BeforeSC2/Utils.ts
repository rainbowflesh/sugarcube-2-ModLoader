...
export class ModUtils {
  ...
  constructor(public pSC2DataManager: SC2DataManager, public thisWin: Window) {}
  ...
  getAddonParamsFromModInfo(
    ...
    addonName: string
  ): ModBootJsonAddonPlugin["params"] | undefined;
  getAddonParamsFromModInfo<P extends any>(
    modInfo: ModInfo,
    addonPluginModName: string,
    addonName: string
  ): P | ModBootJsonAddonPlugin["params"] | undefined {
    return modInfo?.bootJson?.addonPlugin?.find((T) => T.modName === addonPluginModName && T.addonName === addonName)
      ?.params;
  }
}
