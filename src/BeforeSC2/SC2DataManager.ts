...
export class SC2DataManager {
  ...
  checkSC2Data() {
    ...
    const nn = Array.from(rNode.childNodes)
      .map((T) => T.nodeName)
      .filter((T) => T.toUpperCase() !== "TW-PASSAGEDATA");
    if (nn.length !== 2) {
  ...
  async startInit() {
    ...
    console.log(
      "ModLoader ====== mod conflictResult",
      this.conflictResult.map((T) => {
        return {
    ...
    if (!this.getModLoader().checkModCacheData()) {
      ...
      this.getModLoadController().logError(
        "ModLoader ====== SC2DataManager startInit() checkData() fail. Data consistency check failed.. never go there."
      );
    }
    if (!this.getModLoader().checkModCacheUniq()) {
      console.error("ModLoader ====== SC2DataManager startInit() checkNameUniq() fail. never go there.");
      this.getModLoadController().logError(
        "ModLoader ====== SC2DataManager startInit() checkNameUniq() fail. Data consistency check failed.. never go there."
      );
    }
  ...
  async patchModToGame() {
    ...
    // then replace origin
    const modSC2DataInfoCache = replaceMergeSC2DataInfoCache(originSC2DataInfoCache.cloneSC2DataInfo(), em);
    // console.log('patchModToGame() originSC2DataInfoCache', structuredClone(originSC2DataInfoCache.scriptFileItems));
